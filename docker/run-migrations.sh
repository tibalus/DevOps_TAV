...
echo "Connecting to: 34.78.64.72:3306 as user ${AG_DB_USER}"

# Attendre que la connexion soit établie (timeout de 30 secondes)
RETRY_COUNT=0
MAX_RETRIES=15

until mysql -h 34.78.64.72 -P 3306 -u "${AG_DB_USER}" -p"${AG_DB_PASSWORD}" -e "SELECT 1" 2>/dev/null; do
    RETRY_COUNT=$((RETRY_COUNT + 1))
    if [ $RETRY_COUNT -ge $MAX_RETRIES ]; then
        echo "ERROR: Could not connect to database after $MAX_RETRIES attempts"
        echo "Trying one more time with error output..."
        mysql -h 34.78.64.72 -P 3306 -u "${AG_DB_USER}" -p"${AG_DB_PASSWORD}" -e "SELECT 1"
        kill $PROXY_PID
        exit 1
    fi
